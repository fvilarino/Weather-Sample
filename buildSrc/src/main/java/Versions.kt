object Versions {

    object BuildConfig {
        const val compileSdkVersion = 30
        const val minSdkVersion = 23
        const val targetSdkVersion = 30
        const val appVersionCode = 1
        const val appVersionName = "1.0.0"
    }

    object Plugin {
        const val androidGradlePluginVersion = "7.0.0-alpha14"
        const val navigationGradlePluginVersion = "2.3.5"
        const val hiltGradlePluginVersion = "2.33-beta"
    }

    object Kotlin {
        const val kotlinVersion = "1.4.32"
        const val coroutinesVersion = "1.4.3"
        const val ktxVersion = "1.6.0-alpha01"
        const val kotlinSerializationVersion = "1.0.1"
    }

    object Android {
        const val appCompatVersion = "1.2.0"
        const val constraintLayoutVersion = "2.1.0-beta01"
        const val archLifeCycleViewModelVersion = "2.3.1"
        const val lifecycleCommonVersion = "2.4.0-alpha01"
        const val navigationVersion = "2.3.5"
    }

    object Material {
        const val materialVersion = "1.4.0-alpha01"
    }

    object Hilt {
        const val daggerHiltAndroidVersion = "2.33-beta"
        const val daggerHiltAndroidCompilerVersion = "2.33-beta"
        const val androidxHiltLifecycleViewModelVersion = "1.0.0-alpha03"
        const val androidxHiltCompilerVersion = "1.0.0-beta01"
    }

    object Network {
        const val okHttpVersion = "4.9.0"
        const val okHttpInterceptorVersion = "4.9.0"
        const val okIOVersion = "2.10.0"
        const val retrofitVersion = "2.9.0"
        const val retrofitAdapterVersion = "0.9.2"
        const val retrofitSerializationConverterVersion = "0.8.0"
    }

    object Logging {
        const val timberVersion = "4.7.1"
    }

    object Test {
        const val jUnitVersion = "4.13.2"
        const val androidJUnitVersion = "1.1.2"
        const val espressoCoreVersion = "3.3.0"
    }
}
